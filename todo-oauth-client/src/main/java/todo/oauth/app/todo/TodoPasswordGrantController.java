...
/**
 * リソースオーナパスワードクレデンシャルグラントで認可したとき用のコントローラ<br/>
 * わかりやすくするためtodo/passwordにマッピングする
 */
@Controller
@RequestMapping("todo/password")
public class TodoPasswordGrantController {
	// リソースオーナパスワードクレデンシャルグラント用のサービス
	@Inject
	@Named("passwordGrantTodoServiceImpl")
	...
	public String create(@Validated({ Default.class, TodoCreate.class }) TodoForm todoForm, BindingResult bindingResult,
		...
		return "redirect:/todo/password/list";
	}
	...
	@ResponseStatus(HttpStatus.OK)
	public String revokeTokenbyPasswordGrant(RedirectAttributes attributes) {
